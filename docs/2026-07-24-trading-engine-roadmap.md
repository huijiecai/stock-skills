...
### Phase 3B-3进度：证据与综合上下文

完成日期：2026-07-27

- SQLite新增不可变的`catalyst_evidence`证据记录，显式保存来源、发布时间、观察时间、立场和可靠性。
- SQLite新增内容寻址的`context_snapshots`；相同输入生成相同指纹并幂等复用，行情、账户和完整领域上下文均可审计恢复。
- 新增`trader evidence add/list`和`trader context capture/build/replay/show`命令。
- 实时采集自动合并持仓与所有启用观察池成员，只向astock请求本轮决策真正需要的股票代码。
- 综合上下文统一包含账户、持仓估值、预期、观察池、风险敞口、催化证据和阻塞原因。
- 严格执行时间边界：行情、账户、研究状态、买入日期以及证据发布时间和观察时间均不得越过决策时点；未来证据不会进入上下文。
- 持仓缺行情会直接失败；研究信息或观察池行情不完整会生成显式阻塞项，阻塞上下文不能进入判断节点。
- 实时与历史回放使用同一个`DecisionContext`契约和同一个只读判断节点；回放仅消费截止时间之前的分钟行情。
- 新增`trader analyze context`，将上下文ID、指纹、完整输入和结构化提案写入原有判断审计记录。
- 完成隔离数据目录的成功路径和阻塞路径手工验证；45个`trading_engine`测试和4个旧回放信号测试全部通过。

Phase 3尚未完成。剩余工作：

- 接入单个外部LLM provider，并复用已经验证的Pydantic输入输出契约、重试机制和审计记录。

除外部LLM provider外，Phase 3的账户、研究、证据、综合上下文、实时采集、历史回归和审计链路均已完成。
