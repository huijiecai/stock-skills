...
> 状态：Phase 3等待外部LLM；Phase 5结构化上下文已完成
> 创建日期：2026-07-24
...
| 5 | 完整研究工作流 | 归因、预期研究、三维确认和账户级排序 | 进行中 |
...
### Phase 3B-4进度：完整决策上下文v2

完成日期：2026-07-27

- 预期状态新增类型、阶段、催化锚、最短传导链、关联结论和三维确认条件；带交易预案的预期缺少任一关键字段都会阻塞判断。
- 固定池新增`active/dormant/archived`监控状态与直接、量价、相邻、成本压力、研究关系；休眠池仍进入实时采集，可在盘中重新激活。
- 新增结构化日内交易预案，保存动作、数量、优先级、买点/出口类型、观察时点、触发、保护、取消、风险因子和账户排序说明。
- 每个池自动生成涨跌广度、强势数、涨停数、成交额/涨幅排名和领导代码；每只成员保留开盘变化、低点反弹、高点回撤、昨收跌破与修复等价格路径。
- 上下文保留同日关键观察快照、历史判断和已成交记录，使两个观察点、T+1、重复信号及本批机会排序可基于事实判断。
- `context capture`通过astock同时采集全主板异动候选、成交额前列、板块涨幅榜和上证、深证、创业板、上证50、沪深300、中证1000实时指数。
- astock新增`live index`显式指数语义接口，避免`000001`被误识别为股票。
- Paper Broker新增14:50后不开新仓的确定性校验；新仓风险因子来自结构化交易预案，不依赖既有持仓关联。
- 7月27日固定场景回归验证：09:46长城军工500股成交；09:50深科技因长鑫池恢复至3/6且价格反抽而不卖；09:56休眠MLCC池5/5重新确认后风华高科500股成交。现金、风险与T+1状态与旧Skill记录一致。
- 新实现不读取旧Skill文件；旧Skill仅用于本次设计对照，回归测试数据为独立固定夹具。
- 68个`trading_engine`测试、4个旧回放信号测试和astock全量Go测试通过。

Phase 3尚未完成。剩余工作：

- 接入单个外部LLM provider，并复用已经验证的Pydantic输入输出契约、重试机制和审计记录。

除外部LLM provider外，Phase 3的账户、研究、证据、完整决策上下文、实时采集、历史回归、模拟执行和审计链路均已完成。
...
结构化输入和确定性约束已完成：预期类型/阶段、催化传导、固定池关系、三维确认条件、盘前if-then预案、观察历史、市场发现、账户排序所需风险和执行历史均已进入`DecisionContext`。尚未完成的是外部LLM对这些结构化信息的语义归因与提案生成，以及可选的LangGraph连续编排。
...
Phase 0、Phase 1、Phase 2和Phase 4已完成；Phase 3除外部LLM provider外均已完成。Phase 5所需的结构化上下文和硬约束已完成，语义判断部分与外部LLM接入是同一剩余工作。

下一步决定外部LLM provider的供应商、模型、凭证和失败策略，并使用7月27日固定场景验证模型输出。Phase 6实时连续执行仍属于后续路线图，不在当前“完整上下文 + 模拟交易 + LLM”的完成边界内。
