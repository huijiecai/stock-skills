...
async def execute_trade(
    ...
    if action not in ["buy", "sell"]:
        return error_response(400, "action 必须是 buy 或 sell")
    ...
    async with get_db_session() as db:
        ...
        if action == "buy":
            ...
            if not account_row or account_row[0] < amount:
                return error_response(400, "可用资金不足")
            ...
            if not pos_row or pos_row[1] < quantity:
                return error_response(400, "持仓数量不足")
            ...
            new_quantity = pos_row[1] - quantity
            if new_quantity == 0:
                await db.execute(text("DELETE FROM position WHERE id = :id"), {"id": pos_row[0]})
            else:
                await db.execute(
...
async def create_daily_snapshot():
    ...
    async with get_db_session() as db:
        ...
        if exist_result.fetchone():
            return error_response(400, f"{today} 快照已存在")
        ...
        if not account_row:
            return error_response(404, "账户不存在")
        ...
        return success_response(message=f"{today} 快照创建成功")
