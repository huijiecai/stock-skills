...
CREATE TABLE stock_daily (
    ...
    turnover_rate_f REAL,             -- 换手率-自由流通股（小数）
    ...
    ps REAL,                          -- 市销率
    ps_ttm REAL,                      -- 市销率TTM
    dv_ratio REAL,                    -- 股息率（小数）
    dv_ttm REAL,                      -- 股息率TTM（小数）
    -- 股本数据
...
- `market`: 市场代码（SH=上海，SZ=深圳，BJ=北交所）
...
- `turnover`: 成交额，单位为元
- `turnover_rate`: 换手率，小数格式（0.05 = 5%）
- `turnover_rate_f`: 自由流通股换手率，小数格式（更有参考价值）
- `volume_ratio`: 量比
...
- `ps`: 市销率（总市值/营业收入）
- `ps_ttm`: 市销率TTM
- `dv_ratio`: 股息率，小数格式（0.03 = 3%）
- `dv_ttm`: 股息率TTM，小数格式

**股本数据**:
- `total_share`: 总股本，单位万股
- `float_share`: 流通股本，单位万股
- `free_share`: 自由流通股本，单位万股（排除大股东、限售股等）
- `total_mv`: 总市值，单位万元
- `circ_mv`: 流通市值，单位万元
