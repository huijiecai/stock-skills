    ...
    if args.cmd == "run":
        if args.stage in ("live", "replay"):
            (engine.run_live if args.stage == "live" else engine.run_replay)(
                args.system, date=args.date or None)  # type: ignore[arg-type]
        else:
            engine.run_single(args.system, args.stage, date=args.date, topic=args.topic)
        ...
        engine.run_single("expectation", "premarket", date=args.date, prev=args.prev_date)
    elif args.cmd == "close":
        engine.run_single("expectation", "close", date=args.date)
    elif args.cmd == "research":
        engine.run_single("expectation", "research", topic=args.topic)
    else:
        engine.run_live("expectation", sleep_seconds=args.sleep, max_rounds=args.max_rounds)
