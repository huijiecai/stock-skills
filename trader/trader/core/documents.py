...
class Documents:
    ...
    def save(self, doc_type: str, content: str, name: str = "",
             ...
             meta: dict | None = None, bag_id: int = 0) -> int:
        """写入/更新(同 doc_type+name+trade_date+bag_id 覆盖,旧版进 versions)。返回文档 id。"""
        ...
        with _connect(self.schema) as conn:
            row = conn.execute(
                ...
                (doc_type, name, trade_date or "", bag_id),
            ).fetchone()
            if row:
                conn.execute(
    ...
    def set_meta(self, doc_id: int, meta: dict) -> None:
        ...
        with _connect(self.schema) as conn:
            row = conn.execute("SELECT meta FROM documents WHERE id=%s", (doc_id,)).fetchone()
            if row is None:
    ...
    def list(self, doc_type: str | None = None, trade_date: str | None = None,
             bag_id: int = 0) -> list[dict]:
        """文档概览(id/类型/名称/日期/字数/更新时间/meta),可按类型/日期过滤。"""
        where, params = ["bag_id=%s"], [bag_id]
        if doc_type:
...
def _log_version(conn, subject_type: str, subject_id, action: str, payload: dict) -> None:
    """追加一条版本事件(与业务写操作同事务,业务回滚它也回滚)。"""
    import json as _json
    conn.execute(
        "INSERT INTO versions(subject_type, subject_id, action, payload, ts)"
        " VALUES(%s,%s,%s,%s,%s)",
