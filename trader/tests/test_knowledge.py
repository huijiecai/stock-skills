...
def _exp(request):
    return Expectations(schema=f"t_{request.node.name[:40]}")
...
def test_same_direction_multiple_events(request):
...
def test_duplicate_rejected(request):
    """重复(方向+事件)拒绝写入。"""
    e = _exp(request)
    e.add("存储芯片", "存货涨价", "a", "b", "c", "d", POOL)
...
def test_update_stage_and_invalid(request):
...
def test_invalid_stage_rejected(request):
...
def test_pool_codes_core_filter(request):
...
def test_get_all_counts(request):
...
def test_update_content_fields(request):
...
def test_pool_member_upsert(request):
...
def test_remove_pool_member(request):
    """剔除池成员:重新研究调整池。"""
    e = _exp(request)
    eid = e.add("存储芯片", "存货涨价", "a", "b", "c", "d", POOL)
