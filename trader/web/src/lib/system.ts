...
export function stageIcon(s: string): string {
  if (s === '_system' || s === '(system)') return '⚙️'
...
export function orderedStages(stages: Record<string, any>): [string, any][] {
...
export function inferRunStage(run: any, system: string): string {
...
export function kindLabel(_stage: string, def: any): string {
  if (!def) return ''
