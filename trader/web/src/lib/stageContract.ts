export function sourceValue(source: unknown): string {
  ...
  if (source && typeof source === 'object') {
    ...
    return value.stage && value.output ? `${value.stage}.${value.output}` : ''
  }
  return ''
}

/** Internal manifest key for a newly-created stage. User-facing labels may be Chinese. */
export function nextStageId(stages: Record<string, any>, label: string): string {
...
export function validateStageContracts(stages: Record<string, any>): string[] {
  const errors: string[] = []
  for (const [stageName, stage] of Object.entries(stages)) {
    ...
    for (const [slot, spec] of Object.entries(stage.outputs ?? {}) as [string, any][]) {
    ...
    for (const [slot, spec] of Object.entries(stage.inputs ?? {}) as [string, any][]) {
      ...
      if (!['latest', 'previous', 'recent', 'all'].includes(spec.selector ?? 'latest'))
        errors.push(`${stageName}.${slot}:选择策略无效`)
      if (['previous', 'recent'].includes(spec.selector) && Number(spec.limit ?? 1) < 1)
