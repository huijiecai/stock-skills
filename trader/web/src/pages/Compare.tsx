...
import { pnlColor, pnlArrow, metric } from '../lib/ui'
import './Compare.css'

export default function Compare() {
  const [params] = useSearchParams()
  const ids = (params.get('ids') ?? '').split(',').filter(Boolean)
  const a = useQuery({ queryKey: ['run', ids[0]], queryFn: () => get<RunRow>(`/runs/${ids[0]}`), enabled: !!ids[0] })
  const b = useQuery({ queryKey: ['run', ids[1]], queryFn: () => get<RunRow>(`/runs/${ids[1]}`), enabled: !!ids[1] })
  ...
  if (samePrompt && sameDate) {
    ...
    else { verdict = '两场血统完全一致——没有单一变量,无可归因差异'; attr = '血统一致' }
            ...
            <Card title={<Link to={`/runs/${r.id}`}>{r.slug}</Link>} size="small">
              <div className="metric-row" style={{ gridTemplateColumns: 'repeat(4, 1fr)', margin: 0 }}>
                <div className="metric-card" style={{ boxShadow: 'none', padding: '8px 10px' }}>
                  ...
                  <div className="num" style={{ fontSize: 20, color: pnlColor(metric(r, 'return_pct')) }}>
                    {pnlArrow(metric(r, 'return_pct'))}{Math.abs(metric(r, 'return_pct') ?? 0)}%
                  </div>
                </div>
                <div className="metric-card" style={{ boxShadow: 'none', padding: '8px 10px' }}>
                  <div className="metric-label">回撤</div>
                  <div className="num" style={{ fontSize: 20 }}>{metric(r, 'max_drawdown_pct') ?? '-'}%</div>
                </div>
                <div className="metric-card" style={{ boxShadow: 'none', padding: '8px 10px' }}>
                  <div className="metric-label">胜率</div>
                  <div className="num" style={{ fontSize: 20 }}>{metric(r, 'win_rate') ?? '-'}%</div>
                </div>
                <div className="metric-card" style={{ boxShadow: 'none', padding: '8px 10px' }}>
                  <div className="metric-label">交易</div>
                  <div className="num" style={{ fontSize: 20 }}>{metric(r, 'n_fills') ?? 0}<span className="metric-label"> 笔</span></div>
                </div>
              </div>
            </Card>
          </Col>
