...
import type { SystemRow, RunRow, PortfolioRow, PromptRef, CurveOut, StopOut, Stages } from '../api/types'
import LaunchModal from '../components/LaunchModal'
import { NAV, OP, STATUS } from '../lib/icons'
import { PageState, StatusBadge, pnlColor, metric } from '../lib/ui'
import SystemSwitcher from '../components/SystemSwitcher'
import { stageLabel, systemDisplayName, orderedStages } from '../lib/system'
import './SystemAsset.css'

/** 曲线点的消费面:daily(日频)与 points(笔频)两种来源,取字段并集。 */
type CurveDot = { equity: number; date?: string; ts?: string; pnl?: number; pct?: number; run_id?: number | null }

function EquityCurve({ daily, points, initial, liveDays, height = 270, onOpenRun }: {
  daily?: CurveDot[], points: CurveDot[], initial?: number | null,
  liveDays?: Record<string, RunRow>, height?: number, onOpenRun?: (id: number) => void }) {
  /** 上半=日频净值线,下半=每日盈亏柱(红涨绿跌);有值守的日子日期高亮、
   * 零盈亏也画基线刻点(否则看不出"跑了但没交易"和"没跑"的区别)。
   * 悬停带全高度、<title> 放 <g> 第一个子元素(Safari 才显示 tooltip)。 */
  const series: CurveDot[] = (daily?.length ?? 0) >= 2 ? (daily ?? []) : points.slice(1)
  ...
  const eqs = series.map((p) => p.equity)
  ...
  const dds = series.map((p, i: number) => {
    peak = Math.max(peak, p.equity)
    const dd = peak > 0 ? (peak - p.equity) / peak : 0
    if (dd > ddMax) { ddMax = dd; ddPoint = i }
    return dd
  })
  const path = series.map((p, i: number) => `${i ? 'L' : 'M'}${x(i).toFixed(1)},${y(p.equity).toFixed(1)}`).join(' ')
  const last = eqs[eqs.length - 1]
  const color = last >= initial ? 'var(--up)' : 'var(--down)'
  const dayOf = (p: CurveDot) => (p.date || p.ts || '')
  const runOf = (p: CurveDot) => p.run_id ?? liveDays?.[dayOf(p)?.slice(0, 10)]?.id
  const tip = (p: CurveDot, i: number) => {
  ...
  const maxPnl = Math.max(1, ...series.map((p) => Math.abs(p.pnl ?? 0)))
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      {[0, 0.25, 0.5, 0.75, 1].map(f => (
        <g key={f}>
          <line x1={L} x2={W - R} y1={lineTop + f * (lineBot - lineTop)} y2={lineTop + f * (lineBot - lineTop)} stroke="var(--border)" />
          <text x={L - 8} y={lineTop + 4 + f * (lineBot - lineTop)} fontSize={10} fill="var(--text-3)" textAnchor="end">
      ...
      {ticks.map(i => {
        ...
        return (
          <text key={i} x={x(i)} y={H - 6} fontSize={10}
                fill={live ? 'var(--accent)' : 'var(--text-3)'} fontWeight={live ? 700 : 400}
                textAnchor="middle">{dayOf(series[i]).slice(5, 10)}</text>
        )
      })}
      {/* 盈亏柱区零轴 */}
      <line x1={L} x2={W - R} y1={stripMid} y2={stripMid} stroke="var(--border)" />
      <text x={L - 8} y={stripTop + 4} fontSize={9} fill="var(--text-3)" textAnchor="end">盈</text>
      <text x={L - 8} y={stripTop + stripH} fontSize={9} fill="var(--text-3)" textAnchor="end">亏</text>
      <line x1={L} x2={W - R} y1={y(initial)} y2={y(initial)} stroke="var(--border-strong)" strokeDasharray="4,4" />
      <path d={`${path} L${x(n - 1)},${lineBot} L${L},${lineBot} Z`}
            fill={last >= initial ? 'rgba(217,45,32,.05)' : 'rgba(2,122,72,.05)'} />
      <path d={path} fill="none" stroke={color} strokeWidth={2} strokeLinejoin="round" />
      {series.map((p, i: number) => {
        ...
        return (
          <g key={i} style={{ cursor: rid ? 'pointer' : 'default' }}
            ...
            <circle cx={x(i)} cy={y(p.equity)} r={isDD ? 5 : (rid ? 4 : 2.5)}
                    fill={isDD ? 'var(--down)' : 'var(--surface)'}
...
export default function SystemAsset() {
  ...
  const runs = useQuery({
    ...
    refetchInterval: (q) => (q.state.data ?? []).some((r) => r.status === 'running' || r.status === 'stopping') ? 5000 : 30000,
  })
  const portfolios = useQuery({ queryKey: ['portfolios'], queryFn: () => get<PortfolioRow[]>('/portfolios'), staleTime: 30000 })
  const prompts = useQuery({ queryKey: ['prompts', system], queryFn: () => get<PromptRef[]>(`/systems/${encodeURIComponent(system)}/prompts`) })

  const row = detail.data
  const stages = (row?.manifest?.stages ?? {}) as Stages
  const allRuns: RunRow[] = runs.data ?? []
  const liveRuns = allRuns.filter(r => r.kind === 'live')
  const running = allRuns.find(r => r.status === 'running' || r.status === 'stopping')
  const myPorts = (portfolios.data ?? []).filter((p) => p.system_id === row?.id)
  ...
  async function stopRunning() {
    if (!running) return
    await post<StopOut>(`/runs/${running.id}/stop`)
  ...
  const dailyArr: CurveDot[] = activeCurve.data?.daily ?? []
  const seriesSrc: CurveDot[] = dailyArr.length ? dailyArr : (activeCurve.data?.points ?? [])
  const eqs: number[] = seriesSrc.map((p) => p.equity)
  ...
  const winDays = liveRuns.filter(r => (metric(r, 'return_pct') ?? 0) > 0).length
  ...
  return (
    <div>
      ...
      <div className="ws-top">
        ...
        {running && <StatusBadge tone="live" pulse>运行中</StatusBadge>}
        {row?.status === 'archived' && <StatusBadge>已归档</StatusBadge>}
        <div className="ws-modes">
          <span className="on"><NAV.asset /> 资产</span>
          <span onClick={() => firstStage && nav(`${base}/workbench`)}><OP.tool /> 工作台</span>
        </div>
        <div className="ws-btnrow">
          <button className="ws-btn" onClick={() => nav(`${base}/coach`)}><OP.chat /> 教练</button>
          ...
          {running && <button className="ws-btn danger" onClick={stopRunning}><STATUS.stop /> 停止</button>}
        </div>
      </div>
      ...
      <div className="ws-main">
        ...
        <div className="ws-chartcard">
          ...
          {activePort
            ? <PageState query={activeCurve}>
                <EquityCurve daily={dailyArr} liveDays={liveDays} points={activeCurve.data?.points ?? []} initial={activeCurve.data?.initial} onOpenRun={(id) => nav(`/runs/${id}`)} />
              </PageState>
        ...
        <div className="ws-labhead">
          <span><OP.lab /> 模拟实验室</span>
          <StatusBadge>模拟/实验组合独立核算 · 不进实盘履历</StatusBadge>
...
function LabCard({ port, runs }: { port: PortfolioRow, runs: RunRow[] }) {
  const nav = useNavigate()
  const mine = runs.filter(r => r.portfolio_id === port.id)
  const labCurve = useQuery({
    queryKey: ['labCurve', port.id],
    queryFn: () => get<CurveOut>(`/portfolios/${port.id}/curve`),
  })
  const pts: number[] = (labCurve.data?.points ?? []).map((p) => p.equity)
  ...
  return (
    <div className="ws-expcard" style={{ cursor: mine.length ? 'pointer' : 'default' }}
      ...
      <div className="eh"><OP.lab /> {port.name || `实验 #${port.id}`}
