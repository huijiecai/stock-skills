...
export default function DocsBrowser() {
  ...
  const [mode, setMode] = useState<'type' | 'date'>('type')
  const [open, setOpen] = useState<{ doc_type: string, name: string, date: string } | null>(null)

  const detail = useQuery({ queryKey: ['systemDetail', name], queryFn: () => get(`/systems/${encodeURIComponent(name)}`) })
  const docs = useQuery({ queryKey: ['docsAll'], queryFn: () => get('/docs'), staleTime: 15000 })
...
function DocViewer({ doc, onClose }: { doc: any, onClose: () => void }) {
  const c = useQuery({
    queryKey: ['docContent', doc.doc_type, doc.name, doc.date],
    queryFn: () => get(`/docs/content?doc_type=${doc.doc_type}&name=${encodeURIComponent(doc.name)}&date=${doc.date}`),
  })
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(16,24,40,.45)', zIndex: 1000,
             display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, maxWidth: 760, width: '100%',
        ...
        {c.isLoading ? <Spin /> : (
          <pre style={{ whiteSpace: 'pre-wrap', fontSize: 13, lineHeight: 1.7, margin: 0, fontFamily: 'inherit' }}>
            {c.data?.content ?? '(无内容)'}
          </pre>
        )}
      </div>
